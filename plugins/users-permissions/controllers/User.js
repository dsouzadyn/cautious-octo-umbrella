...
module.exports = {
  ...
  tconfirm: async (ctx, next) => {
    const data = await strapi.plugins['users-permissions'].services.user.tconfirm(ctx.params, ctx.request.body);
    ctx.send(data);
  },
  gstatus: async(ctx, next) => {
    const user = await strapi.plugins['users-permissions'].services.user.gstatus(ctx.params);
